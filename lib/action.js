...
/*when it comes to handling form submissions,Normally in react We can go to the component that contains the form and then we could add the onSubmit prop
    ...
    */
    ...
    export async function shareMeal(prevState,formData){        
        ...
        await saveMeal(meal);
        //revalidate or refersh the cache when add something or change something(otherwise it will show the pre-rendered data(cached during application build))
        /**if we write like this "revalidatePath('/meals');" then it will only revalidate meals page data not the nested page of meals.
         * Here mealslug are also depend on meals,so to revalidate nested page we can add like this " revalidatePath('/meals','layout');" 
         * But here share(nested page) doesn't depend on meals ,so it's not need to be revalidated and
         *  mealslug is dynamic page,and NextJs cache only all static pages not dynamic ,so we don't need to revalidate cachee of mealslug,
         * so here we only need to revalidate meals page. 
         * To revalidate all the pages of our entire website we can write revalidatePath('/','layout');*/
